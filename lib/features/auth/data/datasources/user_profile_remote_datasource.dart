...
class UserProfileRemoteDatasource {
  ...
  static const String _usernameField = 'username_lower';
  static const String _phoneField = 'phone_normalized';

  final FirebaseFirestore _firestore;

  CollectionReference<Map<String, dynamic>> get _collection =>
      _firestore.collection(_collectionName) as CollectionReference<Map<String, dynamic>>;
  ...
  Future<void> updateProfile({
    ...
    if (usernameChanged) {
      final existing = await _firestore.collection(_usernameIndexName).doc(newUsernameLower!).get();
      if (existing.exists && ((existing.data()?['uid'] as String?) != uid)) {
        throw Exception('اسم المستخدم مستخدم بالفعل');
      }
    }
    if (phoneChanged) {
      final existing = await _firestore.collection(_phoneIndexName).doc(newPhoneNormalized!).get();
      if (existing.exists && ((existing.data()?['uid'] as String?) != uid)) {
        throw Exception('رقم الهاتف مستخدم بالفعل');
      }
    }
    ...
    await _firestore.runTransaction((tx) async {
      ...
      if (usernameChanged && newUsernameLower != null) {
      ...
      if (phoneChanged && newPhoneNormalized != null) {
